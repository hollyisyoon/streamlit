...
st.markdown(f"""
    ...
    <div class='callout'> 바쁜 사람들은 1인 셀러들이 사업의 규모와 관계없이 <code>데이터에 기반한 의사결정</code>을 내릴 수 있는 환경을 제공하기 위해 탄생한 툴입니다. <br>비즈니스 성장을 위해서는 운영하고 있는 채널에 대한 전반적인 데이터와 외부 데이터를 통해 문제점을 파악하고 알맞은 액션 플랜을 설계하는 것이 중요합니다. <br>바쁜 사람들은 유통 채널별 리뷰 데이터는 물론 경쟁사 리뷰 데이터에 대한 분석, 외부 데이터를 통한 트렌드 및 키워드 분석 결과를 분석가 없이도 한눈에 찾아 볼 수 있습니다.
    </div>""",
